...
return (
  <div className="space-y-1">
          {!collapsed && (
            <p className="px-2 lg:px-3 pt-1 pb-1.5 sm:pb-2 text-[10px] sm:text-xs font-medium text-sidebar-muted uppercase tracking-wider">
    ...
    {SUBJECT_ORDER.filter((subjectId) => canAccessSubject(subjectId, userRole)).map((subjectId) => {
      ...
      return (
        ...
        >
          ...
          >
            <subject.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </span>
          {!collapsed && (
...
return (
  <div className="space-y-4">
    ...
    >
      <span className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 rounded-lg flex-shrink-0">
    ...
    {!collapsed && (
      <div className="flex items-center gap-2 lg:gap-3 px-2 lg:px-3 py-1.5">
        <span className={cn("flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 rounded-lg flex-shrink-0", subject.iconBg)}>
          <subject.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
        </span>
          ...
          {section.items.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => setMobileOpen(false)}
                className={cn(
                  "flex items-center gap-1.5 sm:gap-2 lg:gap-3 py-1.5 sm:py-2 px-2 lg:px-3 rounded-lg text-xs sm:text-sm transition-colors -ml-px border-l-2",
              ...
              >
                <span className={cn(
                  "flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 rounded-md flex-shrink-0",
                  active ? "text-sidebar-primary" : "text-sidebar-muted"
                )}>
                  <item.icon className="w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4" />
                </span>
                {!collapsed && (
...
const SidebarContent = () => (
  <>
    {/* Logo */}
    <div className="shrink-0 flex items-center gap-2 sm:gap-3 px-2.5 sm:px-3 lg:px-4 py-3 sm:py-4 lg:py-6 border-b border-sidebar-border">
      <div className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 lg:w-10 lg:h-10 rounded-xl bg-sidebar-primary flex-shrink-0">
        <Smartphone className="w-4 h-4 sm:w-4.5 sm:h-4.5 lg:w-5 lg:h-5 text-sidebar-primary-foreground" />
      </div>
      {!collapsed && (
    ...
    <nav className="sidebar-nav-scroll flex-1 min-h-0 overflow-y-auto px-2 lg:px-3 py-2 sm:py-3 lg:py-4">
      {subjectsMode ? (
        <SubjectsNav
          language={language}
          collapsed={collapsed}
...
return (
  <>
    ...
    <aside
      className={cn(
        "lg:hidden fixed inset-y-0 left-0 z-50 w-[260px] sm:w-[280px] min-w-[260px] sm:min-w-[280px] max-w-[80vw] sm:max-w-[85vw] bg-sidebar flex flex-col transition-transform duration-300 ease-in-out shadow-xl",
